/*!
* \file 
* \brief Файл исходного кода программы
* 
* Файл содержит в себе реализацию функций программы, необходимых для раскрытия скобок в математическом выражении,
* заданном деревом разбора выражений. В их числе функция, решающая главную задачу, а также функции, необхожимые для:
* непосредственно раскрытия скобок в дереве разбора выражений, копирования вершины и всех её производных в вектор вершин,
* заполнения дерева пустыми вершинами, сравнения деревьев между собой, перестраиванию части дерева ниже заданного узла
* в соответсвтии со знаком его операции, проверки входных данных на наличие ошибок и вывода сообщений о них, 
* чтения входных данных из файла и записи выходных данных в файл.
*/
...
bool crashOutput(ErrorType error)
{
    ...
    switch (error)
    {
        ...
        case OUTPUT_FILE_CREATION_FAILED:
            cerr << "Неверно указан файл для выходных данных. Возможно указанного расположения не существует или нет прав на запись." << endl;
            isSuccessOutput = true;
            break;

        case WRONG_DATA_FORMAT:
            cerr << "Программа принимает на вход одно дерево разбора выражений в виде списка смежности. Измените входной файл." << endl;
            isSuccessOutput = true;
            break;

        case NOT_BINARY_TREE:
            cerr << "Дерево разбора выражений должно быть бинарным. Измените дерево разбора выражений." << endl;
            isSuccessOutput = true;
            break;

        case SAME_NODES_ID:
            cerr << "Узлы дерева разбора выражений не уникальны. Измените дерево разбора выражений на корректное." << endl;
            isSuccessOutput = true;
            break;

        case SAME_PARENT_AND_NODE_ID:
            cerr << "Узел дерева разбора выражений не может являться родительским самому себе. Измените дерево разбора выражений на корректное." << endl;
            isSuccessOutput = true;
            break;

        case LACK_OF_OPERANDS:
            cerr << "Поддерживаемые операции всегда бинарны. Добавьте операнды, где их недостаёт." << endl;
            isSuccessOutput = true;
            break;

        case NODE_ID_OUT_OF_RANGE:
            cerr << "Идентификатором узла должно являться натуральное число в диапазоне [1..30]. Измените идентификаторы узлов дерева разбора выражений на корректные." << endl;
            isSuccessOutput = true;
            break;

        case PARENT_ID_OUT_OF_RANGE:
            cerr << "Идентификатором родительского узла для узла в дереве разбора выражений должно являться целое ненулевое число в диапазоне [-1..29]. Измените идентификаторы узлов дерева разбора выражений на корректные." << endl;
            isSuccessOutput = true;
            break;

        case INVALID_NODE_SYMBOL:
            cerr << "Дерево разбора выражений задано некорректно. В значении операнда используется недопустимый символ." << endl;
            isSuccessOutput = true;
            break;

        case NOT_RIGHT_LINK:
            cerr << "Дерево разбора выражений задано не корректно. Родители и дети совпадают не для всех узлов." << endl;
            isSuccessOutput = true;
            break;

        case UNEXCEPTABLE_PARENT_ID:
            cerr << "Родителем узла дерева разбора выражений не может являться его ребёнок. Измените дерево разбора выражений." << endl;
            isSuccessOutput = true;
            break;

        case NO_ROOT:
            cerr << "В дереве разбора выражений отсутствует корень. Добавьте его." << endl;
            isSuccessOutput = true;
            break;

        case TREE_SIZE_OUT_OF_RANGE:
            cerr << "Число узлов в дереве разбора выражений должно быть натуральным числом в диапазоне [1..30]." << endl;
            isSuccessOutput = true;
            break;

        case INVALID_NUMBER_OF_NODES:
            cerr << "Дерево разбора выражений задано некорректно. Указанное и фактическое число узлов в дереве разбора выражений не совпадают." << endl;
            isSuccessOutput = true;
            break;
        default:
            cerr << "Произошла неизвестная ошибка. Попробуйте перезапустить программу." << endl;
            isSuccessOutput = true;
            break;
    }
